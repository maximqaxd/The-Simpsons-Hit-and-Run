...
)
{
    ...
    {
        // C++20: path::u8string() returns std::u8string (char8_t), so convert explicitly.
        const std::u8string u8filename = directoryEntry.filename().u8string();
        std::string filename;
        filename.reserve(u8filename.size());
        for (char8_t ch : u8filename)
